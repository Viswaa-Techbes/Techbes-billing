      ...
      <div className="card-panel p-5 rounded-2xl bg-white border border-slate-200 text-xs print:hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-3 text-slate-650">
          ...
          <div>
            ...
            <span>Issue: {document.issueDate && !isNaN(new Date(document.issueDate).getTime()) ? new Date(document.issueDate).toLocaleDateString('en-IN') : '—'} | Valid till: {document.validTill && !isNaN(new Date(document.validTill).getTime()) ? new Date(document.validTill).toLocaleDateString('en-IN') : '—'}</span>
          </div>
          <div>
