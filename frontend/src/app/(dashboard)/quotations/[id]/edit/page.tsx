...
export default function EditQuotationPage() {
  ...
  const addItemLine = () => {
    const newItem = createBlankItemLine();
              ...
              <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 text-[11px] font-bold bg-slate-100 text-slate-600 rounded-full">
                    {items.filter(isUsefulItemLine).length}
                  </span>
                </div>
                <div className="flex gap-2">
                  <button
                  ...
                  <table className="w-full text-left text-xs border-collapse min-w-[860px]">
                    <thead className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200 shadow-xs">
                      <tr className="text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                        ...
                        <th className="px-3 py-3 w-24 min-w-[85px] text-center">GST %</th>
                        <th className="px-3 py-3 w-40 min-w-[145px] text-right">Total (INR)</th>
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {items.map((item, idx) => {
                        ...
                        return (
                          <tr key={item.id || idx} id={`line-row-${item.id}`} className="align-top hover:bg-slate-50/40 transition-colors">
                            {/* Item Details */}
                            ...
                            <td className="px-3 py-3 align-top w-28 min-w-[100px]">
                              <input
                                ...
                                className="w-full form-input text-xs py-1 text-center qty-input font-medium"
                              />
                            </td>
                            ...
                            {/* Total Amount */}
                            <td className="px-3 py-3 text-right font-bold text-slate-800 align-top pt-5 w-40 min-w-[145px] whitespace-nowrap font-mono">
                              ₹{computedItem.total?.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
            ...
            <div className="border-t border-slate-200 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button
                  ...
                  disabled={saving}
                  className="w-full sm:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-sm flex items-center justify-center gap-2 text-xs disabled:opacity-60"
                >
                  Review Quotation
                </button>
              </div>
          ...
          <div className="border-t border-slate-200 pt-6 grid grid-cols-1 md:grid-cols-2 gap-8 text-[11px] text-slate-650 font-sans">
            ...
            <div className="border-t border-slate-200 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <button
                type="button"
                onClick={() => setActiveStep(1)}
                className="w-full sm:w-auto px-5 py-2.5 border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-semibold shadow-xs transition-colors"
              >
                Back to Editor
              </button>
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <Link
                  href={`/quotations/${id}`}
                  className="flex-1 sm:flex-none px-5 py-2.5 border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-semibold shadow-xs transition-colors text-center"
                >
                  Cancel
                </Link>
                <button
                  ...
                  onClick={handleUpdate}
                  disabled={saving}
                  className="flex-1 sm:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-sm transition-colors disabled:opacity-60"
                >
                  {saving ? 'Saving...' : 'Save Changes'}
                </button>
              </div>
