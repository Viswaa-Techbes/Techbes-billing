...
import { toIndianWords } from '@/utils/numberToWords'; // wait! is it available in frontend? 
// The numberToWords.js utility was written in backend. Let's copy it here in a frontend util or define it locally.
// Let's write the toIndianWords locally inside page.tsx to ensure no build error!
...
export default function NewQuotationPage() {
    ...
    const loadConfiguration = async () => {
      try {
        ...
        if (clientsRes.data?.success) {
          ...
          // Check query parameters to preselect client
          const queryClientId = searchParams.get('clientId');
          if (queryClientId) {
            const matched = clientList.find((c: any) => c._id === queryClientId);
            if (matched) {
              setSelectedClientId(queryClientId);
              setSelectedClient(matched);
              
              // Set default place of supply
              if (matched.billingAddress?.state) {
                setPlaceOfSupply({
                  state: matched.billingAddress.state,
                  stateCode: matched.billingAddress.stateCode || '',
                });
              }
              if (matched.shippingAddress) {
                setShippingAddress({
                  addressLine1: matched.shippingAddress.addressLine1 || '',
                  addressLine2: matched.shippingAddress.addressLine2 || '',
                  city: matched.shippingAddress.city || '',
                  state: matched.shippingAddress.state || '',
                  stateCode: matched.shippingAddress.stateCode || '',
                  country: matched.shippingAddress.country || 'India',
                  pincode: matched.shippingAddress.pincode || '',
                });
              }
            }
          }
        ...
        showToast('Failed to load profile or clients configuration.', 'error');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async (statusOverride?: 'DRAFT' | 'SENT') => {
    if (!businessProfile) {
      showToast('Please complete your Business Profile first.', 'error');
      return;
    }
    if (!selectedClientId) {
  ...
  // If business profile is missing, show warning
  if (!businessProfile || !businessProfile.businessName) {
    return (
      <div className="max-w-xl mx-auto mt-12 text-slate-800 animate-slideUp">
        <div className="card-panel p-8 rounded-xl border border-red-200 bg-red-50/20 text-center space-y-4">
          <div className="w-12 h-12 rounded-full bg-red-50 text-red-500 flex items-center justify-center mx-auto">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h3 className="text-base font-bold text-slate-800">Complete your business profile</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
            You must complete your business settings and profile details (such as address, state, and company name) before creating quotations and estimates.
          </p>
          <Link
            href="/settings"
            className="inline-block px-5 py-2.5 bg-blue-600 hover:bg-blue-750 text-white rounded-xl text-xs font-semibold shadow-sm transition-colors"
          >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500 font-medium">
            <span className={activeStep === 1 ? 'text-brand-primary font-bold' : ''}>1. Quotation Details</span>
        ...
        <div className="flex gap-2">
          ...
          {activeStep === 1 ? (
            <button
              onClick={() => {
                if (!selectedClientId) {
                  showToast('Please select a Client to review.', 'error');
                  return;
                }
                setActiveStep(2);
              }}
      ...
      {activeStep === 1 ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 items-start">
          ...
          <div className="xl:col-span-3 card-panel p-8 rounded-xl space-y-8 bg-white">
              ...
              <div className="text-left sm:text-right space-y-1.5 text-xs text-slate-650 font-mono">
                ...
                <div className="flex items-center sm:justify-end gap-2">
                  ...
                  <input
                    ...
                    className="border border-slate-300 rounded px-1.5 py-0.5 text-slate-900 w-28 bg-white"
                  />
                </div>
                <div className="flex items-center sm:justify-end gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-4">
              ...
              <div className="space-y-3">
                ...
                <div className="text-xs text-slate-600 space-y-1">
                  ...
                  {businessProfile.pan && <p className="font-mono">PAN: {businessProfile.pan}</p>}
            ...
            <div className="border-t border-slate-100 pt-5 space-y-4">
              <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer select-none">
                <input
                  ...
                  className="rounded border-slate-300 text-brand-primary w-4 h-4 focus:ring-0"
                />
            ...
            <div className="border-t border-slate-100 pt-5 grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
              ...
              <div className="text-xs md:pl-6 pt-4 md:pt-0">
                <span className="font-semibold text-slate-500">GST Mode: </span>
            ...
            <div className="border-t border-slate-100 pt-6 space-y-4">
                ...
                <table className="w-full text-left text-xs border-collapse min-w-[900px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {items.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} className="align-top hover:bg-slate-50/20 transition-colors">
                          <td className="p-3">
                            <input
                              type="text"
                              value={item.itemName}
                          ...
                          <td className="p-3 text-center align-middle">
                            <div className="flex items-center justify-center gap-1">
                              <button
                                ...
                                className="p-1 text-slate-450 hover:text-blue-600 rounded hover:bg-slate-100"
                              ...
                              <button
                                ...
                                className="p-1 text-slate-450 hover:text-rose-500 rounded hover:bg-slate-100 disabled:opacity-30"
                                title="Remove Line"
                              >
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                type="button"
                onClick={addItemLine}
                className="px-4 py-2 border border-dashed border-slate-300 hover:border-brand-primary text-slate-600 hover:text-brand-primary rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {/* Document Level discount & additional charges */}
            ...
            {/* Expandable Sections (Terms, Notes, Key-Values, Signatory Name) */}
            <div className="border-t border-slate-100 pt-6 space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Payment terms, delivery clauses, etc..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Special remarks for client view..."
                  className="w-full form-input text-xs h-20 resize-y"
                />
              </div>
                    ...
                    {customFields.map((field, idx) => (
                      <div key={idx} className="flex gap-2 items-center text-xs">
                        ...
                        <input
                          ...
                          placeholder="Field Value (e.g. PRJ-2026)"
                          className="form-input text-xs py-1.5 flex-1"
                        />
                        <button
                          type="button"
          ...
          <div className="card-panel p-6 rounded-xl space-y-6 bg-white text-xs text-slate-700">
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Currency</h4>
              <select className="w-full form-input text-xs bg-white py-1.5" disabled>
        ...
        <div className="card-panel max-w-4xl mx-auto rounded-xl p-12 bg-white shadow-lg space-y-8 animate-fadeIn text-xs text-slate-800 font-sans border border-slate-200">
          <div className="flex justify-between items-start border-b border-slate-200 pb-6">
            <div>
              <h2 className="text-xl font-bold tracking-tight text-slate-900">QUOTATION</h2>
              <p className="text-[10px] text-slate-400 font-medium tracking-wide mt-1 uppercase">TechBes Billing Service</p>
                  ...
                  <tr key={idx} className="align-top py-2.5">
                    <td className="py-2.5">
                      ...
                      {displayOptions.showItemDescriptions && item.description && (
                        <p className="text-[10px] text-slate-450 mt-0.5 leading-relaxed">{item.description}</p>
                      )}
                    </td>
                    {displayOptions.showHsnSac && (
                    ...
                    <td className="py-2.5 text-center text-slate-600">{item.gstRate}%</td>
